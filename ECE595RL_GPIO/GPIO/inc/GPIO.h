...
/**
 * @brief The LED_Pattern_1 function sets the output of the user LEDs and the eight LEDs on the PMOD 8LD module based on the status of the user buttons.
 *
 * This function sets the output of the built-in red LED (P1.0), the RGB LED (P2.0 - P2.2), and the PMOD 8LD (P9.0 - P9.7)
 * based on the status of the Button 1 (P1.1) and Button 2 (P1.4).
 *
 ...
 *  -------------           -----------             -------------              -----------
 *      0x00            Toggle Red Every 1s      Toggle Green Every 1s           ALL OFF
 *      0x10                    Red                     Off                 EVEN ON, ODD OFF
 *      0x02                    Off                     Pink                EVEN OFF, ODD ON
 *      0x12                    Off                     Off                      ALL ON
 *
 *
 * @return None
 ...
 * and decrements down to 0 with a delay of 100 ms between each count and repeats indefinitely.
 * The LED pattern occurs when only SWT2 on the PMOD SWT module is turned on.
 *
 *
 * @param None
 *
 ...
 *
 * This function turns toggles LED1 every 1s, toggles the RGB LED with a blue color every 1s,
 * and then toggles all LEDs on the PMOD 8LD module every 1s.
 * The LED pattern occurs when only SWT3 on the PMOD SWT module is turned on.
 *
 *
 * @param None
 *
 ...
 * and then initiates a binary x2 multiplier pattern on the PMOD 8LD module. The counter starts from 1 (0x01)
 * and doubles every 500ms up to 128 (0x80) and then repeats indefinitely.
 * The LED pattern occurs when only SWT4 on the PMOD SWT module is turned on.
 *
 *
 * @param None
 *
